...
def main():
    ...
    with open(args.csv) as csvfile:
        ...
        for row in reader:
            if row['spot_id'] and row['duration'] == '!':
                tracks.append(row['spot_id'])

    scope = "playlist-modify-private"
    sp = spotipy.Spotify(auth_manager=SpotifyOAuth(scope=scope))
    user_id = sp.me()['id']
    splits  = [tracks[i:i+10] for i in range(0, len(tracks), 10)]
    for s in splits:
        sp.user_playlist_remove_all_occurrences_of_tracks(user_id, args.playlist, s)
